...
export const prepareManagerAppOptinTransactions = (managerAppId : number, getMaxAtomicOptInMarketAppIds : number[], sender: string, storageAddress : string, suggestedParams : SuggestedParams) => {
    //need to figure out how to get OPT_INT... * 1000000 to work without getting typescript error for using parseInt
    let txns = [];
    //payment txn
    txns.push(algosdk.makePaymentTxnWithSuggestedParams(sender, storageAddress, OPT_IN_MIN_BALANCE * 1000000, _, _, suggestedParams));
    ...
    txns.push(algosdk.makeApplicationOptInTxn(storageAddress, suggestedParams, managerAppId,_, _, _, _, _, _, appAddress));
...
export const prepareMarketAppOptinTransactions = (marketAppId : number, sender : string, suggestedParams : SuggestedParams) => {
